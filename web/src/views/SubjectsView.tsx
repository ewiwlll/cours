...
export const SubjectsView: React.FC<SubjectsViewProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'courses' | 'flashcards'>('courses');
  ...
  const filteredSubjectsList = useMemo(() => {
    ...
    return subjects.filter((s) => s.category?.trim() === selectedCategory);
              ...
              <div className="space-y-5">
                ...
                {subjectChapters.map((ch) => {
                  ...
                  return (
                    ...
                    >
                      {/* Header du Chapitre */}
                      <div className="w-full p-4 sm:p-5 flex items-center justify-between bg-surface-elevated/40 hover:bg-surface-elevated transition-colors text-left select-none">
                        <button
                      ...
                      {/* Contenu déroulé : Les séances / cours du chapitre */}
                      {!isCollapsed && (
                        <div className="p-4 sm:p-5 space-y-3 bg-surface/50 border-t border-border/60">
                          {chCourses.length === 0 ? (
                            <div className="py-6 text-center text-xs text-zinc-500 bg-background/50 rounded-xl border border-border-subtle">
                              Aucune séance dans ce chapitre pour le moment. Cliquez sur <strong>« + Séance »</strong> pour en ajouter une.
                            </div>
                          ) : (
                            [...chCourses]
                ...
                {/* 2. COURS SANS CHAPITRE (TOUT EN BAS DE LA PAGE) */}
                    ...
                    <div className="space-y-3">
                      {coursesByChapter.unassigned.map((c) => (
                        <div
                          key={c.id}
                          onClick={() => onOpenCourse?.(c.id)}
                          className="p-4 rounded-xl bg-surface border border-border/80 hover:border-zinc-700 hover:bg-surface-elevated/60 cursor-pointer transition-all flex items-center justify-between gap-4 group"
                        >
